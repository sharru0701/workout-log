...
test("스토어 공개 프로그램 목록과 테스트 매트릭스가 일치한다", async ({ page }, testInfo) => {
  ...
  await expect(cards).toHaveCount(PROGRAMS.length, { timeout: 20_000 });
  const actualNames = await cards.evaluateAll((items) =>
    items.map((item) => item.getAttribute("aria-label") ?? ""),
  );
  expect(new Set(actualNames)).toEqual(new Set(PROGRAMS.map((program) => program.inventoryName)));
...
for (const scenario of PROGRAMS) {
  test(`${scenario.inventoryName}: 화면 시작 → 전 세트 입력 → 저장 → 재진입`, async ({ page }, testInfo) => {
    const browserFailures = observeBrowser(page);
