...
test("formatPerformedHistoryLine: load 컨텍스트로 맨몸 총무게 병기", () => {
  // compact (uniform)
  assert.equal(
    formatPerformedHistoryLine(
      [
      ...
      { exerciseName: "Pull-Up", bodyweightKg: 74, locale: "ko" },
    ),
    "105kg (+31) × 5 × 3",
  );
  // 비-맨몸은 병기 없음
  assert.equal(
    formatPerformedHistoryLine(
      [
        { weightKg: 72.5, reps: 5 },
        { weightKg: 72.5, reps: 5 },
      ],
      { exerciseName: "Back Squat", bodyweightKg: 74, locale: "ko" },
    ),
    "72.5kg × 5 × 2",
  );
  // expanded (non-uniform) — 세트별 병기
  assert.equal(
    formatPerformedHistoryLine(
      [
        { weightKg: 105, reps: 5 },
