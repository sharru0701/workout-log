...
import type { FailureProtocolChoice } from "@/shared/ui/failure-protocol-sheet";
...
const FailureProtocolSheet = dynamic(() => import("@/shared/ui/failure-protocol-sheet").then(mod => mod.FailureProtocolSheet), { ssr: false });
...
type WorkoutRecordPageProps = {
  initialPlans?: PlanItem[];
  initialSettings?: import("@/lib/settings/workout-preferences").SettingsSnapshot | null;
};

export function WorkoutRecordPage({ initialPlans, initialSettings }: WorkoutRecordPageProps = {}) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        // PERF: 서버가 초기 플랜+설정 데이터를 내려준 경우 클라이언트 fetch 스킵 (RTT #1 제거)
        let planRes: PlansResponse;
        let settingsSnapshot: import("@/lib/settings/workout-preferences").SettingsSnapshot | null;
        if (initialPlans != null) {
          planRes = { items: initialPlans };
          settingsSnapshot = initialSettings ?? null;
        } else {
          [planRes, settingsSnapshot] = await Promise.all([
