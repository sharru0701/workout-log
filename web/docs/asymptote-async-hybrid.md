...
### 3.4 TM 배수 보정 (절충) — *상수 노출*

초기 TM = 최근 추정 1RM × **0.87** (Asymptote 0.83 ↔ Async 공격적의 절충). TB에서 0.90 TM을
이미 소화 중인 리프터의 재적응 낭비를 줄이고 스쿼트 강도를 의미 있는 수준으로 끌어올린다.

- 코드: `ASYMPTOTE_HYBRID_TM_PERCENT = 0.87` 상수. 플랜 초기화(1RM→TM) 단계에서 참조.

### 3.5 e1RM 연속 모니터 (Async 차용) — *구현됨 (순수 헬퍼)*
...
체중 기록이 전제** — 미기록이면 풀업 추세가 노이즈가 된다.

- 코드: `program-engine/asymptote-monitor.ts` — `asymptoteDriverTrend(exposures, window=7)`가 드라이버
  노출 시계열에서 e1RM·이동평균·추세(RISING/FLAT/FALLING/INSUFFICIENT)를 산출하는 순수 함수.
  `e1rm-service`의 탑세트 행을 입력으로 먹일 수 있다. UI 노출은 후속.
...
| e1RM 연속 모니터 | `program-engine/asymptote-monitor.ts` `asymptoteDriverTrend` (순수 헬퍼) | 구현 |
| TM 배수 | `asymptote.ts` `ASYMPTOTE_HYBRID_TM_PERCENT` (=0.87) | 상수 노출 (init 연동은 후속) |
| 테스트 | `asymptote-hybrid.test.ts`, `asymptote-monitor.test.ts`, `progression/asymptote-trigger-deload.test.ts` | 구현 |
...
> 프로그램·preview 경로 동작 불변). 남은 후속: e1RM 모니터 UI 노출, TM×0.87 init 경로 연동.
