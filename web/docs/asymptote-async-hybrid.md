...
### 3.3 트리거 조기 디로드 (Async 차용) — *문서 규칙 (운영)*

고정 사이클4(바닥 보험)는 유지하되, 그 전이라도 *3 드라이버(SQ-A/BP-C/WPU-A) 중 2개가 최근
2회 노출에서 그라인딩으로 렙 급감* 시 즉시 사이클4(디로드)로 점프한다. 불규칙 공백이 이미
회복을 주는 패턴이면 고정 디로드를 앞당겨 낭비를 줄인다.
...
### 3.5 e1RM 연속 모니터 (Async 차용) — *운영/분석*
...
| 연속일 AMRAP 가드 | `program-engine/asymptote.ts` `asymptoteShouldDeferAmrap` / `asymptoteSetGuidance`, 처방의 `params.restDayGap` 연동 | 구현 |
...
| 트리거 디로드 / e1RM 모니터 | — | 운영 규칙 (엔진 후속) |
| 테스트 | `program-engine/asymptote-hybrid.test.ts` | 구현 |

> `restDayGap`은 처방 생성 시 직전 세션 `performed_at`과의 일 간격으로 채운다. 현재 `GeneratorCtx`엔
> 미연동이며, 값이 없으면 가드는 비활성(기존 동작 보존)이다. 연동은 처방 부트스트랩 레이어에서
> `params.restDayGap` 주입으로 추가하는 후속 작업.
