...
| 체중 확인 안내 | `sessionHasBodyweightExercise`(중량풀업이 있는 모든 프로그램 게이트, 14일 스테일) + `BodyweightCheckBanner`("업데이트/유지" 권고). 저장 시 체중을 `totalLoadKg`로 스탬프(모든 프로그램 중량풀업) | 구현 |
| 테스트 | `asymptote-hybrid.test.ts`, `asymptote-monitor.test.ts`, `progression/asymptote-trigger-deload.test.ts`, `lib/program-store/model.test.ts`, `lib/bodyweight-load.test.ts` | 구현 |
...
| 0.4 | 2026-06-13 | 검증(풀업 AMRAP) 세션 진입 시 체중 "업데이트/유지" 안내 배너(매번 입력 대신 중요한 순간에만 권고). 갱신은 prefs.bodyweight.kg → 모니터가 읽음. |
| 0.5 | 2026-07-07 | 프라이밍 탑세트(SQ/PULL/BP, 사이클2·3, coef 1.0, stopOnGrind, 진행 무영향) — 근접도 공백(≥80% 노출 0회) 해소. 세션A 스쿼트 계수 다이얼 스테이징 플래그. 진입 1RM 실측값 확정. 상세: [proximity patch 스펙](asymptote-hybrid-v0.5-proximity-patch.md) |
| 0.5.1 | 2026-07-08 | 실패 프로토콜 피드백: 조기디로드 이벤트화+배너, 블록 판정 리포트, AMRAP 보류 배너, 라이트 블록 배지, AMRAP 전날 예고. 엔진 판정 로직 무변경. 상세: [feedback patch 스펙](asymptote-hybrid-v0.5.1-feedback-patch.md) |
