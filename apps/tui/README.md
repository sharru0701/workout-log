...
- **today / 진행 무게** — Operator 6주·5/3/1 4주 블록 종료와 Starting Strength·StrongLifts·Texas 3연속 실패, Greyskull 2연속 실패에서 운동별 다음 무게를 확인·수정한 뒤 저장합니다. 진행 제외 보조운동은 판정하지 않습니다.
...
- **programs / REF5** — 템플릿 생성 시 IANA 시간대 → 최근 기록/추정 1RM 추천 또는 고급 직접 입력 → 첫 처방 확인. e1RM은 저장하지 않고 확정된 직접 kg만 계획에 남깁니다. REF5 플랜에서 `v` 상태(NEXT/표준/윈도/LOCK/MICRO), `R` 새로고침.
