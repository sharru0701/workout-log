...
// TestLiveLogRPE proves a fractional RPE round-trips: create a log with rpe=8.5 and
...
func TestLiveLogRPE(t *testing.T) {
	...
	for _, lg := range logs {
		...
		if *lg.Sets[0].RPE != 8.5 {
			t.Errorf("RPE = %g, want 8.5", *lg.Sets[0].RPE)
		}
		t.Logf("RPE round-trip ok: rpe=%g", *lg.Sets[0].RPE)
		return
	}
